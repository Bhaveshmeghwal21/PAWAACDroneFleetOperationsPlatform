 ...
 * this test exercises the real HTTP stack (controller + trace middleware) with
 * lightweight stubs for the readiness indicators — no Postgres/Redis required.
 * Datastore-backed readiness is covered separately by Testcontainers specs.
 */
...
describe('Health (e2e)', () => {
  ...
  it('GET /health returns 200 with liveness payload', async () => {
    ...
    expect(res.body).toMatchObject({ status: 'ok', service: 'alert-notification' });
